...
public class GetGoldResourceDetailsStrategy(IActionDatabasePort actionDatabase) : IGetResourceDetailsStrategy
{
    public async Task<Dictionary<ResourceDetailsType, Dictionary<string, int>>> Execute(GetResourceDetailsQuery input, CancellationToken cancellationToken)
    {
        ...
        {
            var usedGold = await actionDatabase.ComputeUsed(ResourceType.Gold, input.UserId, actionType);
            usedDict.Add(actionType.ToString(), usedGold);
            totalGoldUsed += usedGold;
        }
        usedDict.Add("Total", totalGoldUsed);
        
        
        productionDict.Add("Production", await actionDatabase.ComputeTotal(ResourceType.Gold, input.UserId));
        productionDict.Add("Available", productionDict["Production"] - totalGoldUsed);
